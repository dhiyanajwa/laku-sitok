...
function CustomerMenuPage() {
  ...
  async function submitOrder() {
    ...
    try {
      const { data } = await createOrder({ customerName, items: cartItems.map((item) => ({ productId: item.id, quantity: item.quantity })) })
      setConfirmation({ ...data.data, customerName }); setCartItems([]); setCustomerName(''); setCartOpen(false); loadProducts()
  ...
  return <Box sx={{ minHeight: '100vh', bgcolor: 'background.default' }}>
    ...
    <Container maxWidth="md" sx={{ py: { xs: 4, md: 6 } }}>
      <Stack spacing={1} sx={{ mb: 4 }}><Typography component="h1" variant="h3" fontWeight={900}>Warung Murni</Typography><Typography color="text.secondary">Order your favourites. We will prepare them fresh for you.</Typography></Stack>
    ...
    <Dialog open={Boolean(confirmation)} onClose={() => setConfirmation(null)} maxWidth="xs" fullWidth>
      <DialogTitle>Order received</DialogTitle><DialogContent><Stack spacing={2.25} alignItems="center" sx={{ pb: 1, textAlign: 'center' }}>
        ...
        <Typography variant="h5" fontWeight={900} color="primary">{confirmation?.orderNumber}</Typography><Typography color="text.secondary">Usually ready in 10–15 minutes.</Typography>
        {trackingUrl && <QRCodeSVG value={trackingUrl} size={144} includeMargin />}
        <Button variant="contained" fullWidth onClick={() => navigate(`/track/${confirmation.trackingToken}`)}>Track my order</Button>
        <Button variant="outlined" fullWidth onClick={copyTrackingLink}>{copied ? 'Tracking link copied' : 'Copy tracking link'}</Button>
        <Button onClick={() => setConfirmation(null)}>Back to menu</Button>
      </Stack></DialogContent>
    </Dialog>
  </Box>
}
