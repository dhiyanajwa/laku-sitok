...
export async function postOrder(request, response) { const vendorId = await getVendorId(request.body.vendorId); response.status(201).json({ data: await placeOrder(vendorId, request.body) }) }
